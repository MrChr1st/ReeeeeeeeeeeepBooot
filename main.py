...
def _write_last_sent(value: str) -> None:
    Path(STATE_FILE).write_text(value, encoding="utf-8")


def _stats_text(storage: SharedStorage) -> str:
...
async def auto_sender(bot: Bot, storage: SharedStorage, private_chat_id: str, hours: int) -> None:
    while True:
        try:
            if private_chat_id:
                ...
                if should_send:
                    ...
                    document = FSInputFile(
                        file_path,
                        filename=f"report_24h_{now.strftime('%Y%m%d_%H%M%S')}.xlsx",
                    )
                    await bot.send_document(
                        chat_id=private_chat_id,
                        document=document,
                        caption="📊 Автоотчёт Excel за 24 часа",
                    )
                    await bot.send_message(chat_id=private_chat_id, text=_stats_text(storage))
                    _write_last_sent(now.strftime("%Y-%m-%d %H:%M:%S"))
        except Exception as exc:
            logging.exception("[reportbot auto_sender] failed: %s", exc)
